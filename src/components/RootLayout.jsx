          ...
          >
            <Icon
              className={clsx(
                'h-6 w-6',
                invert
                  ? 'fill-white group-hover:fill-slate-200'
...
function NavigationItem({ href, children }) {
  return (
    <Link
      href={href}
      className="group relative isolate -mx-6 bg-slate-950 px-6 py-10 even:mt-px sm:mx-0 sm:px-0 sm:py-16 sm:odd:pl-16 sm:even:mt-0 sm:even:border-r sm:even:border-slate-800 sm:even:pr-16"
    >
      {children}
      <span className="absolute inset-y-0 -z-10 w-screen bg-slate-900 opacity-0 transition group-odd:left-0 group-even:right-0 group-hover:opacity-100" />
    </Link>
  )
}
